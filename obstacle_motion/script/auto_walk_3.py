...
pub_vel = rospy.Publisher('cmd_vel/man_3', Twist, queue_size=1)
sub_odom = rospy.Subscriber("odom/man_3", Odometry, Set_Odom)
run_rate = 5 # hz
vel_ahead = 1.5
vel_turn = 1.0
correction_para = 0.1
pos = [0,0]
front = 0
points = [[-3.7,-9.3], [-3.7,-1], [-3.7,-9.3], [-1.5,-9.3]]
points_counter = 0
run_state = 0
...
def run_points(data):
    ...
    if points_counter != pre_points_counter or run_state != pre_run_state:
        print ("man 333 for {} state : {}".format(data[points_counter], run_state))
...
def main():
    rospy.init_node('walking_man_3', anonymous=True)
    rate = rospy.Rate(5)
